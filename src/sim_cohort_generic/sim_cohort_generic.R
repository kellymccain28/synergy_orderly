...
params_df <- params_df <- data.frame(
  max_SMC_kill_rate = rep(3, orderlyparams$n_param_sets),
  lambda = rep(13.08, orderlyparams$n_param_sets),
  kappa = rep(0.43, orderlyparams$n_param_sets)
)
...
# SMC delivery 
season_start_day <- 212 # August 1 in the prob_bit_generic dataset
season_length <- 120
params_df <- params_df %>%
  rowwise() %>%
...
output_dir = paste0('simulation_outputs', Sys.Date(), '/')
...
saveRDS(results2, paste0('results_', Sys.Date(),'.rds'))
...
formattedinfrecords <- format_model_output(model_data = o$infection_records, 
                                           cohort = 'generic',
                                           simulation = params_row$sim_id)
testinci <- get_incidence(df_children = metadata_df, 
                          casedata = o$infection_records)
