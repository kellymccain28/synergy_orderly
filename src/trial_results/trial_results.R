...
primary <- readRDS(paste0(pathtodata,'cleaned/primary_cleaned.rds'))
children <- readRDS(paste0(pathtodata,'cleaned/children_cleaned.rds'))
mitt <- readRDS(paste0(pathtodata, 'cleaned/mitt_cleaned.rds')) 
mitt <- remove_labels(mitt)
primary_pt <- read_dta(paste0(pathtodata, 'from Paul/primary_outcome_person_time_file.dta')) %>%
...
ag_model_smcref <- coxph(Surv(start_time, 
                       end_time, 
...
# Display results
summary(ag_model_smcref)

coxsumm<-as.data.frame(summary(ag_model_smcref)$coefficients)

tidy_results_smcref <- tidy(ag_model_smcref,
                             exponentiate = TRUE,
                             conf.int = TRUE) %>%
...
ag_model_rtssref <- coxph(Surv(start_time, 
                              end_time, 
...
# Display results
summary(ag_model_rtssref)

coxsumm2<-as.data.frame(summary(ag_model_rtssref)$coefficients)

tidy_results_rtssref <- tidy(ag_model_rtssref,
                             exponentiate = TRUE,
                             conf.int = TRUE) %>%
  # Calculate vaccine efficacy and its confidence intervals
  mutate(
...
tidy_results_rtssref


# Plot the vaccine efficacies 
tidy_results <- rbind(tidy_results_rtssref, tidy_results_smcref)

ggplot(tidy_results %>% filter(term %in% c("Both vs. RTSS",'RTSS vs. SMC','Both vs. SMC')))+
...
ggsurvplot(kmsurvobj, #group.by = "country",
           # linetype = "strata",
           facet.by = 'country',
